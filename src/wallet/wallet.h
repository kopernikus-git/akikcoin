...
extern bool fGlobalUnlockSpendCache; // Bool used for letting the precomputing thread know that zakcspends need to use the cs_spendcache
...
// Possible states for zAKC send
enum ZerocoinSpendStatus {
    ZAKC_SPEND_OKAY = 0,                            // No error
    ZAKC_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZAKC_WALLET_LOCKED = 2,                         // Wallet was locked
    ZAKC_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZAKC_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZAKC_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZAKC_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZAKC_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZAKC_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZAKC_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZAKC_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZAKC_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZAKC_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZAKC_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZAKC_SPENT_USED_ZAKC = 14,                      // Coin has already been spend
    ZAKC_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZAKC_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    bool CreateZAKCOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    string GetUniqueWalletBackupName(bool fzakcAuto) const;
    ...
    */
    ...
    bool fBackupMints;
    std::unique_ptr<CzAKCTracker> zakcTracker;
    ...
    void setZWallet(CzAKCWallet* zwallet)
    {
        zwalletMain = zwallet;
        zakcTracker = std::unique_ptr<CzAKCTracker>(new CzAKCTracker(strWalletFile));
    }

    CzAKCWallet* getZWallet() { return zwalletMain; }
    ...
    /** zAKC reset */
    boost::signals2::signal<void()> NotifyzAKCReset;
