...
QList<TransactionRecord> TransactionRecord::decomposeTransaction(const CWallet* wallet, const CWalletTx& wtx)
{
    ...
    if (wtx.HasZerocoinSpendInputs()) {
        ...
        if (wtx.HasZerocoinPublicSpendInputs()) {
            ...
            if (!ZAKCModule::ParseZerocoinPublicSpend(wtx.vin[0], wtx, state, publicSpend)){
    ...
    if (wtx.IsCoinStake()) {
        ...
        if (wtx.HasZerocoinSpendInputs() && (fZSpendFromMe || wallet->zakcTracker->HasMintTx(hash))) {
            //zAKC stake reward
            sub.involvesWatchAddress = false;
            sub.type = TransactionRecord::StakeZAKC;
...
bool IsZAKCType(TransactionRecord::Type type)
{
    switch (type) {
        case TransactionRecord::StakeZAKC:
...
void TransactionRecord::updateStatus(const CWalletTx& wtx)
{
    ...
    else if (type == TransactionRecord::Generated || type == TransactionRecord::StakeMint || type == TransactionRecord::StakeZAKC || type == TransactionRecord::MNReward) {
