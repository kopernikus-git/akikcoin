...
CzAKCWallet::CzAKCWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zakc seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZAKCSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZAKCSeed_deprecated()) {
                    LogPrintf("%s: Updated zAKC seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zakc seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zakc wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzAKCWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZAKCCount(nCountLastUsed);
    else if (!walletdb.ReadZAKCCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzAKCWallet::Lock()
{
    seedMaster = 0;
}

void CzAKCWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzAKCWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZAKC(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzAKCWallet::LoadMintPoolFromDB()
{
...
void CzAKCWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzAKCWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzAKCWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zakcTracker->HasPubcoinHash(pMint.first)) {
...
bool CzAKCWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZAKC(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zakcTracker which also adds to database
    pwalletMain->zakcTracker->Add(dMint, true);
...
void CzAKCWallet::SeedToZAKC(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzAKCWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzAKCWallet::GenerateDeterministicZAKC(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzAKCWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzAKCWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzAKCWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
