...
CzAKCTracker::CzAKCTracker(std::string strWalletFile)
{
...
CzAKCTracker::~CzAKCTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzAKCTracker::Init()
{
...
bool CzAKCTracker::Archive(CMintMeta& meta)
{
...
bool CzAKCTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzAKCTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzAKCTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzAKCTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzAKCTracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzAKCTracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CzAKCTracker::GetSerialHashes()
{
...
CAmount CzAKCTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzAKCTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzAKCTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzAKCTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzAKCTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzAKCTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzAKCTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzAKCTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzAKCTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzAKCTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzAKCTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzAKCWallet* zAKCWallet)
{
    bool iszAKCWalletInitialized = (NULL != zAKCWallet);
    CMintMeta meta;
    ...
    if (! iszAKCWalletInitialized)
        zAKCWallet = new CzAKCWallet(strWalletFile);
    meta.isSeedCorrect = zAKCWallet->CheckSeed(dMint);
    if (! iszAKCWalletInitialized)
        delete zAKCWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzAKCTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzAKCTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzAKCTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzAKCTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzAKCTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzAKCTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzAKCWallet* zAKCWallet = new CzAKCWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zAKCWallet);
        }
        delete zAKCWallet;
        LogPrint("zero", "%s: added %d dzakc from DB\n", __func__, listDeterministicDB.size());
    }
