...
#include "ui_zakccontroldialog.h"
...
void ZPivControlDialog::updateList()
{
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        {
            LOCK(pwalletMain->zakcTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->zakcTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zAKC.";
            else if (!mint.isSeedCorrect)
                strReason = "The zAKC seed used to mint this zAKC is not the same as currently hold in the wallet";
            else
