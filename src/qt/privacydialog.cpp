                                                          ...
                                                          fDenomsMinimized(true)
{
    ...
    ui->zAKCpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzAKCSyncStatus->setText("(" + tr("out of sync") + ")");
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zAKCpayAmount->setFocus();
    }
}
/* disable MINT
 *
void PrivacyDialog::on_pushButtonMintzAKC_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if (encStatus == walletModel->Locked) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Mint_zAKC, true));
        if (!ctx.isValid()) {
...
void PrivacyDialog::on_pushButtonSpendzAKC_clicked()
{
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Send_zAKC, true));
        if (!ctx.isValid()) {
            // Unlock wallet was cancelled
            return;
        }
...
void PrivacyDialog::sendzAKC()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Akikcoin Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zAKCpayAmount->text().toDouble();
    ...
    if (walletModel && walletModel->getAddressTableModel()) {
        // If zAKC was spent successfully update the addressbook with the label
    ...
    // Clear zakc selector in case it was used
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    std::vector<CMintMeta> vMints = pwalletMain->zakcTracker->GetMints(false);
    ...
    ui->labelzAvailableAmount_4->setText(QString::number(zerocoinBalance/COIN) + QString(" zAKC "));
    ...
    ui->labelZsupplyAmount->setText(QString::number(chainActive.Tip()->GetZerocoinSupply()/COIN) + QString(" <b>zAKC </b> "));
    ui->labelZsupplyAmount_2->setText(QString::number(chainActive.Tip()->GetZerocoinSupply()/COIN) + QString(" <b>zAKC </b> "));
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        ...
        QString strSupply = QString::number(nSupply) + " x " + QString::number(denom) + " = <b>" +
                            QString::number(nSupply*denom) + " zAKC </b> ";
        switch (denom) {
            case libzerocoin::CoinDenomination::ZQ_ONE:
...
{
    ui->labelzAKCSyncStatus->setVisible(fShow);
}
...
void PrivacyDialog::updateSPORK16Status()
{
    ...
    //bool fButtonsEnabled =  ui->pushButtonMintzAKC->isEnabled();
    ...
    if (fMaintenanceMode && fButtonsEnabled) {
        // Mint zAKC
        //ui->pushButtonMintzAKC->setEnabled(false);
        //ui->pushButtonMintzAKC->setToolTip(tr("zAKC is currently disabled due to maintenance."));

        // Spend zAKC
        ui->pushButtonSpendzAKC->setEnabled(false);
        ui->pushButtonSpendzAKC->setToolTip(tr("zAKC is currently disabled due to maintenance."));
    } else if (!fMaintenanceMode && !fButtonsEnabled) {
        // Mint zAKC
        //ui->pushButtonMintzAKC->setEnabled(true);
        //ui->pushButtonMintzAKC->setToolTip(tr("PrivacyDialog", "Enter an amount of AKC to convert to zAKC", 0));

        // Spend zAKC
        ui->pushButtonSpendzAKC->setEnabled(true);
        ui->pushButtonSpendzAKC->setToolTip(tr("Spend Zerocoin. Without 'Pay To:' address creates payments to yourself."));
    }
}
