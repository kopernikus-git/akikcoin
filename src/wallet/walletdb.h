...
public:
    ...
    bool WriteZAKCSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZAKCSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZAKCSeed_deprecated(uint256& seed);
    bool EraseZAKCSeed();
    bool EraseZAKCSeed_deprecated();

    bool WriteZAKCCount(const uint32_t& nCount);
    bool ReadZAKCCount(uint32_t& nCount);
