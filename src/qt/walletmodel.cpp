...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered akikcoin address / amount:
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        foreach (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal akikcoin:URI (akikcoin:XyZ...?message=example)
            {
...
static void NotifyzAKCReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzAKCReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzAKCReset.connect(boost::bind(NotifyzAKCReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzAKCReset.disconnect(boost::bind(NotifyzAKCReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->zakcTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus, fWrongSeed);
}
