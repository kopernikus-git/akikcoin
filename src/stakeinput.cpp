...
// The zAKC block index is the first appearance of the accumulator checksum that was used in the spend
...
{
    //The unique identifier for a zAKC is a hash of the serial
...
{
    //Create an output returning the zAKC that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZAKCOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zAKC output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zAKC", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZAKCOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zAKC output", __func__);
...
{
    CzAKCTracker* zakcTracker = pwallet->zakcTracker.get();
    CMintMeta meta;
    if (!zakcTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zakcTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}
...
{
    //The unique identifier for a AKC stake is the outpoint
