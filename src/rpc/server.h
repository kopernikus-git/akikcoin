...
/**
 * Akikcoin RPC command dispatcher.
 */
class CRPCTable
{
private:
...
extern UniValue DoZakcSpend(const CAmount nAmount, bool fMintChange, bool fMinimizeChange, vector<CZerocoinMint>& vMintsSelected, std::string address_str, bool isPublicSpend = true);
...
extern UniValue setzakcseed(const UniValue& params, bool fHelp);
extern UniValue getzakcseed(const UniValue& params, bool fHelp);
extern UniValue generatemintlist(const UniValue& params, bool fHelp);
extern UniValue searchdzakc(const UniValue& params, bool fHelp);
extern UniValue dzakcstate(const UniValue& params, bool fHelp);
